...
var tableName = "elements_has_regions";
var tableAttributes = ["elements_id", "regions_id"];
...
exports.getByIds = function (element_id, region_id, done) {
    var sql = squel.select()
        .from(tableName);
    var expr = squel.expr();
    if (element_id != null) {
    ...
    if (region_id != null) {
        expr.and(tableAttributes[1] + " = ?", region_id);
    }
    sql.where(expr);
...
exports.getWithCreation = function (element_id, region_id, done, conn) {
    var tempConn = conn;
    if (conn == null) {
        tempConn = db.get();
    }
    var sql = squel.insert()
        ...
        .set(tableAttributes[1], region_id);
    ...
    var getSql = squel.select()
        ...
        squel.expr()
            ...
            .and(tableAttributes[1] + " = ?", region_id)
    );
