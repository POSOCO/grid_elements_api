...
var tableName = "elements_has_states";
var tableAttributes = ["elements_id", "states_id"];
...
exports.getByIds = function (element_id, state_id, done) {
    var sql = squel.select()
        .from(tableName);
    var expr = squel.expr();
    if (element_id != null) {
    ...
    if (state_id != null) {
        expr.and(tableAttributes[1] + " = ?", state_id);
    }
    sql.where(expr);
...
exports.getWithCreation = function (element_id, state_id, done, conn) {
    var tempConn = conn;
    if (conn == null) {
        tempConn = db.get();
    }
    var sql = squel.insert()
        ...
        .set(tableAttributes[1], state_id);
    ...
    var getSql = squel.select()
        ...
        squel.expr()
            ...
            .and(tableAttributes[1] + " = ?", state_id)
    );
