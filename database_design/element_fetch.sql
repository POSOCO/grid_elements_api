...
FROM
  (SELECT
     ...
     voltages.level,
     element_types.type,
     GROUP_CONCAT(owners1.name ORDER BY owners1.name ASC SEPARATOR '/')   AS el_owners_list,
     GROUP_CONCAT(regions1.name ORDER BY regions1.name ASC SEPARATOR '/') AS el_regions_list,
         ...
         (
           ...
           FROM substations
             ...
             (SELECT
                elements.*,
                GROUP_CONCAT(owners1.name ORDER BY owners1.name ASC SEPARATOR '/')  AS el_owners_list,
                GROUP_CONCAT(regions1.name ORDER BY owners1.name ASC SEPARATOR '/') AS el_regions_list,
                voltages.level
              FROM elements
                LEFT OUTER JOIN voltages ON voltages.id = elements.voltages_id
                LEFT OUTER JOIN (
...
WHERE elems_table.name LIKE '%wardha%' AND elems_table.type = 'line'